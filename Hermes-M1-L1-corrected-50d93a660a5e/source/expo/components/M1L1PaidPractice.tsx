...
export function M1L1PaidPractice({ requestedRunId, convertedLesson, onReturnToDeck, onDiscard, onSafetyExit }: M1L1PaidPracticeProps): React.JSX.Element {
  ...
  const persist = useCallback(async (next: PersistedScenarioPracticeRun): Promise<void> => {
    await replaceActiveScenarioRunStrict(next);
    setValue(next);
  }, [replaceActiveScenarioRunStrict]);

  const run = value?.run;
  const lesson = run?.m1L1;
  const currentPressure = lesson?.coachedBeat === 3 ? lesson.pushbackOne : lesson?.pushbackTwo;
  ...
  const confirmSecondResponse = useCallback(async (): Promise<void> => {
    ...
    const note = m1L1CoachNote(draft, 5);
    if (!note) return;
  ...
  const confirmRetry = useCallback(async (isFinal: boolean): Promise<void> => {
    ...
    const original = lesson.coachedBeat === 3 ? run?.responseAttempt?.transcript : lesson.secondResponseAttempt?.transcript;
    ...
    const next = {
      ...
      run: {
        ...retried.run,
        state: shouldOfferFinal ? "ready_for_final_retry" as const : "attempt_comparison" as const,
        comparison,
        updatedAt: Date.now(),
      },
    };
    await persist(next);
    setDraft("");
  }, [convertedLesson.retryCap, draft, lesson, persist, run?.responseAttempt?.transcript, value]);

  const replayFlaggedPressure = useCallback(async (nextState: "ready_for_retry" | "ready_for_final_retry"): Promise<void> => {
    if (!value || !currentPressure) return;
    await persist(transitionScenarioPracticeRun(value, nextState, Date.now()));
    await speakPilotAudio(lineFor(currentPressure));
  }, [currentPressure, persist, value]);
  ...
  return <View style={styles.root}><Backdrop />
    ...
    <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <ScrollView contentContainerStyle={[styles.scroll, { paddingBottom: insets.bottom + 120 }]} keyboardShouldPersistTaps="handled">
        ...
        <Pressable onPress={onSafetyExit} style={styles.safety} accessibilityRole="button"><Text style={styles.safetyText}>This doesn’t feel safe to practice</Text></Pressable>
        ...
        {state === "hope_coaching" && currentPressure ? <Reveal><StatusPill label="Hope · one observed behavior" tone="purple" /><ProductCard accent style={styles.card}><Text style={styles.body}>{run.coachNote}</Text><SectionLabel tone={C.purple}>Exact-moment retry</SectionLabel><Text style={styles.body}>{run.retryInstruction}</Text></ProductCard><PrimaryButton label="Replay the exact flagged pressure" onPress={() => void replayFlaggedPressure("ready_for_retry")} containerStyle={styles.action} /></Reveal> : null}
        ...
        {state === "ready_for_final_retry" && currentPressure ? <Reveal><StatusPill label="Optional final retry · 2 of 2" tone="amber" /><Text style={styles.title}>One final retry is available.</Text><Text style={styles.body}>You can try the same exact pressure once more, or continue to the behavior-only comparison.</Text><PrimaryButton label="Replay and retry once more" onPress={() => void replayFlaggedPressure("ready_for_final_retry")} containerStyle={styles.action} /><Pressable onPress={() => void persist(transitionScenarioPracticeRun(value, "attempt_comparison", Date.now()))} style={styles.secondary}><Text style={styles.secondaryText}>Continue without another retry</Text></Pressable><Capture title="Final retry" kind="final-retry" value={draft} onChange={setDraft} onRecord={requestCapture} onType={typeCapture} /></Reveal> : null}
