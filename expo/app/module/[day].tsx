...
export default function PilotModuleScreen() {
  ...
  const chooseCarriedContext = useCallback(async (): Promise<void> => {
    if (day === 1 || session?.scenarioSource !== "user_supplied") {
      router.push("/custom");
      return;
    }
    const prepared = await ensureSessionAndRun();
  ...
  }, [day, ensureSessionAndRun, persistRun, router, session?.scenarioSource]);
  ...
  const chooseQuiz = useCallback(async (choice: "A" | "B"): Promise<void> => {
    ...
    await persistRun({ ...run, quizChoice: choice, updatedAt: Date.now() });
  }, [persistRun, run]);

  const submitQuiz = useCallback(async (): Promise<void> => {
    if (!run || !quizChoice) return;
    await persistRun({ ...transitionPilotRun(run, "quiz_feedback"), quizChoice });
  }, [persistRun, quizChoice, run]);
...
const styles = StyleSheet.create({
  ...
  lessonProposition: { ...T.body, color: C.textSoft, marginTop: 10 }, conceptCard: { marginTop: 14, padding: 20 }, conceptMark: { width: 48, height: 48, borderRadius: 24, backgroundColor: C.purpleSoft, alignItems: "center", justifyContent: "center", marginBottom: 6 }, conceptNumber: { fontFamily: font.bold, fontSize: 17, color: C.purple }, conceptText: { fontFamily: font.medium, fontSize: 18, lineHeight: 27, color: C.text, marginTop: 8 },
  ...
  contrast: { gap: 10, marginTop: 18 }, contrastCell: { minHeight: 108, borderRadius: radius.md, borderWidth: 1, borderColor: "rgba(177,64,47,0.18)", backgroundColor: "rgba(255,255,255,0.58)", padding: 14, gap: 8 }, contrastStrong: { borderColor: "rgba(92,138,110,0.24)", backgroundColor: "rgba(255,255,255,0.82)" }, contrastText: { fontFamily: font.regular, fontSize: 14, lineHeight: 21, color: C.text },
  optionList: { gap: 11, marginTop: 22 }, answer: { minHeight: 92, borderRadius: radius.md, backgroundColor: "rgba(255,255,255,0.72)", borderWidth: 1, borderColor: C.line, padding: 15, flexDirection: "row", alignItems: "center", gap: 12, ...shadow.layer }, answerSelected: { borderColor: C.purple, backgroundColor: C.purpleSoft }, answerPressed: { borderColor: "rgba(81,40,136,0.34)", backgroundColor: C.purpleSoft, transform: [{ scale: 0.988 }] }, answerLetter: { width: 30, height: 30, borderRadius: 15, borderWidth: 1, borderColor: C.lineStrong, alignItems: "center", justifyContent: "center" }, answerLetterSelected: { backgroundColor: C.purple, borderColor: C.purple }, answerLetterText: { fontFamily: font.semi, fontSize: 12, color: C.purple }, answerLetterTextSelected: { color: C.onAccent }, answerText: { ...T.support, color: C.text, flex: 1 }, answerAudio: { width: 44, height: 44, borderRadius: 22, backgroundColor: C.purpleSoft, alignItems: "center", justifyContent: "center" },
