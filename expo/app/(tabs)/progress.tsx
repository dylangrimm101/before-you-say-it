      ...
      <ScrollView contentContainerStyle={[styles.scroll, { paddingTop: insets.top + 18, paddingBottom: insets.bottom + 112 }]} showsVerticalScrollIndicator={false}>
        ...
        <Reveal index={1}>
          <GlassCard style={styles.indexCard}>
            ...
            <Text style={styles.indexNote}>{evidence.indexValue === null ? "There is not enough approved evidence for an Index value yet." : "This value averages observed signals only. Unobserved signals are not treated as zero."}</Text>
          </GlassCard>
        </Reveal>

        <Reveal index={2}>
          <View style={styles.signalSection}><Text style={styles.sectionLabel}>SIGNAL COVERAGE</Text>{evidence.rows.map((row) => { const observed = row.value !== null; return <View key={row.key} style={styles.signalRow}><View style={styles.signalHeading}>{observed ? <Check size={14} color={C.sage} strokeWidth={2.8} /> : <Circle size={14} color={C.track} />}<Text style={[styles.signalName, observed && styles.signalObserved]}>{row.label}</Text><Text style={styles.signalValue}>{row.value ?? "Not observed"}</Text></View>{observed ? <><Meter value={row.value ?? 0} tone={C.purple} height={5} /><Text style={styles.evidenceMeta}>{row.evidenceTurnIds.length} approved evidence turn{row.evidenceTurnIds.length === 1 ? "" : "s"}</Text></> : null}</View>; })}</View>
        </Reveal>
...
const styles = StyleSheet.create({
  ...
  signalSection: { marginTop: 30 }, sectionLabel: { ...eyebrow, color: C.dim, marginTop: 30, marginBottom: 10 }, signalRow: { paddingVertical: 11, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: C.line }, signalHeading: { flexDirection: "row", alignItems: "center", gap: 9, marginBottom: 7 }, signalName: { ...T.support, color: C.dim, flex: 1 }, signalObserved: { color: C.text, fontFamily: font.semi }, signalValue: { ...T.caption, color: C.purple }, evidenceMeta: { ...T.caption, color: C.dim, marginTop: 6 },
