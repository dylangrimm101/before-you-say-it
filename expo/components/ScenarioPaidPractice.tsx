...
export function ScenarioPaidPractice({ scenario, requestedRunId }: ScenarioPaidPracticeProps) {
  ...
  if (!value || !context || !run) {
    return <View style={[styles.root, styles.center]}><Backdrop /><Text style={styles.title}>This scenario run is unavailable.</Text><Text style={styles.body}>Return to Scenarios and start a fresh rehearsal. No generic practice fixture was substituted.</Text><PrimaryButton label="Back to Scenarios" onPress={() => router.replace("/(tabs)/library")} containerStyle={styles.action} /></View>;
  }
...
function CounterpartCard({ name, role, turnId, text }: { name: string; role: string; turnId: string; text: string }) {
  return <ProductCard accent style={styles.counterpartCard}><SectionLabel tone={C.purple}>{name} · {role}</SectionLabel><Text style={styles.counterpartText}>“{text}”</Text><Text style={styles.turnId}>Same pressure turn · {turnId}</Text></ProductCard>;
}
...
const styles = StyleSheet.create({
  ...
  listening: { alignItems: "center", gap: 20, paddingTop: 36 }, busy: { marginTop: 14 }, processing: { minHeight: 56, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 12 }, counterpartCard: { marginTop: 18 }, counterpartText: { ...T.body, marginTop: 10 }, turnId: { ...T.caption, marginTop: 12 }, coachCard: { marginTop: 14, gap: 8 }, comparison: { marginTop: 10 },
});
