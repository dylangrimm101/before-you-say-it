...
await terms();
await press('Continue to account'); assert.equal(path, '/continue-from-web');
...
await reset(); await terms(); await press('Continue to account'); await press('Create an account to save this result'); await fill();
...
await reset(); await terms(); await press('Continue to account'); await fill(); loginMode = 'claim-problem';
...
assert.ok(button('Continue to account').props.disabled); assert.ok(!text().includes('$7.49'), 'never fabricate unavailable store pricing');
...
await reset(); await terms(); await press('Continue to account'); hasCurrentGuestPractice = false; await render();
await press('Create an account'); assert.ok(text().includes('Create your account'), 'account creation remains available without a current guest result');
