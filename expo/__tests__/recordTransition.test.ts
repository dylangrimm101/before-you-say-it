...
for(const track of ['real','recurring','skill'])for(const recovery of ['keep-reading','voice-retry']){
  test(`second voice failure reaches debrief: ${track}/${recovery}`,()=>{
    const result=spawnSync(process.execPath,['__tests__/recordTransition.fixture.ts','fresh',track,'continue','voice-failure',recovery],{cwd:import.meta.dir+'/..',encoding:'utf8'});
