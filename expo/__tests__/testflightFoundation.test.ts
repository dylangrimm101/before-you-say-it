...
describe("TestFlight foundation", () => {
  ...
  test("privacy copy matches optional account and RevenueCat identity behavior", async () => {
    ...
    expect(privacy).toContain("account credentials use protected device storage");
    expect(privacy).toContain("Signing in reconnects your eligible access");
    expect(privacy).toContain("subject to a current access check");
    expect(privacy).toContain("does not automatically sync between devices");
    expect(privacy).toContain("including approved conversation content and its result");
    expect(privacy).toContain("expiry does not promise immediate physical erasure");
    expect(privacy).toContain("Other local app data is not separately encrypted by BYSI");
