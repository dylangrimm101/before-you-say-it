...
try {
 ...
 const badQuote='You said "twice this month", but that does not settle the handoff.';
 outputs=[{...retained.response,text:badQuote},{...retained.response,text:positive}];
 ...
 assert.equal(JSON.parse(provider[1].messages[1].content).text,badQuote,'repair sees unchanged rejected quote');
 ...
 assert.deepEqual(prompt.lesson_sources.untrusted_utterances,[{id:'user_turn_1',speaker:'learner',text:learner}]);
 assert.equal(prompt.lesson_sources.background.scene,retained.request.contract.scenario);
 assert.ok(!JSON.stringify(prompt.lesson_sources).includes(badQuote));
