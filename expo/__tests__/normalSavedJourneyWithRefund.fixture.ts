...
try{
 ...
 // Independent parent assertion: one-time source refunds never clear native Pro.
await db.exec(readFileSync(web+'/server/follow-through/schema.sql','utf8'));
const {createRuntime:createFollowThroughRuntime}=await import(web+'/server/follow-through/runtime.mjs');
let ftCheckout:any,ftRefunds:any[]=[];let ftCalls=0;
const ftFetch=async(url:any,init:any)=>{
 const u=String(url);
 if(u===authOrigin+'/auth/v1/user')return authFetch(url,init);
 if(u==='https://api.stripe.com/v1/checkout/sessions'&&init.method==='POST'){
  const p=new URLSearchParams(init.body);
  ftCheckout={id:'cs_cross_source',url:'https://checkout.stripe.com/c/pay/synthetic',mode:'payment',payment_status:'paid',livemode:false,metadata:{binding:p.get('metadata[binding]')},line_items:{has_more:false,data:[{price:{id:'price_cross_source'},quantity:1}]},payment_intent:{id:'pi_cross_source',status:'succeeded',latest_charge:{id:'ch_cross_source',payment_intent:'pi_cross_source',livemode:false,captured:true,amount_captured:999,paid:true,amount:999,currency:'usd'}}};return Response.json(ftCheckout);
 }
 if(u.startsWith('https://api.stripe.com/v1/checkout/sessions/cs_cross_source'))return Response.json(ftCheckout);
 if(u.startsWith('https://api.stripe.com/v1/refunds?'))return Response.json({has_more:false,data:ftRefunds});
 throw Error('Unexpected cross-source synthetic transport '+u);
};
const ft=await createFollowThroughRuntime({database:contentDatabase,env:{BYSI_FOLLOW_THROUGH:'owner-v1',BYSI_FOLLOW_THROUGH_LIVEMODE:'false',BYSI_NATIVE_SERVICE_DATABASE_URL:'postgresql://bysi_native_service:synthetic@db.spvksnddzyvycfoefrcf.supabase.co:5432/postgres?sslmode=verify-full',BYSI_NATIVE_AUTH_ORIGIN:authOrigin,BYSI_NATIVE_PUBLISHABLE_KEY:'synthetic',STRIPE_SECRET_KEY:'synthetic',STRIPE_BYSI_FOLLOW_THROUGH_PRICE_ID:'price_cross_source'},fetch:ftFetch});
const ftToken='Bearer '+tokenFor(owner);const ftInput={operationId:'44444444-4444-4444-8444-444444444444',intent:'Synthetic pickup',rawMessage:'Can we choose one task?',negativePattern:'Waiting',situationType:'logistics',forecast:{diagnosis:'synthetic original'}};
const ftResult={mode:'repair_plan',outcome:'Exact original synthetic Follow-Through result'};
const ftSource=await ft.checkout(ftToken,ftInput);
assert.deepEqual(await ft.deliver(ftToken,ftSource.sessionId,async()=>{ftCalls++;return ftResult;}),ftResult);
const makeRefund=(id:string,status:string,amount:number)=>({id,status,amount,charge:'ch_cross_source',payment_intent:'pi_cross_source',currency:'usd'});
const sourceEvent={id:'evt_cross_source',type:'charge.refunded',data:{object:{id:'ch_cross_source',payment_intent:'pi_cross_source'}}};
for(const rs of [[makeRefund('re_partial','succeeded',100)],[makeRefund('re_pending','pending',999)],[makeRefund('re_failed','failed',999)]]){
 ftRefunds=rs;await ft.reconcileEvent(sourceEvent);
 assert.equal((await db.query('select denied from bysi_follow_through.purchase where owner_id=$1',[owner])).rows[0].denied,false);
 assert.equal(await (await import('../lib/nativeBillingRuntime')).nativeBilling!.access(),true);
}
ftRefunds=[makeRefund('re_partial','succeeded',100),makeRefund('re_rest','succeeded',899)];await ft.reconcileEvent(sourceEvent);
assert.equal((await db.query('select denied from bysi_follow_through.purchase where owner_id=$1',[owner])).rows[0].denied,true);
assert.equal(await (await import('../lib/nativeBillingRuntime')).nativeBilling!.access(),true,'independent native entitlement survives full one-time refund');
assert.deepEqual(await ft.deliver(ftToken,ftSource.sessionId,async()=>{ftCalls++;throw Error('must not regenerate');}),ftResult);
ftRefunds=[];await ft.reconcileEvent({...sourceEvent,id:'evt_stale_positive'});
assert.equal((await db.query('select denied from bysi_follow_through.purchase where owner_id=$1',[owner])).rows[0].denied,true,'stale positive cannot resurrect original source');
assert.equal(ftCalls,1);assert.deepEqual((await db.query('select state from bysi_native_free.session where owner_id=$1',[owner])).rows[0].state.record,savedBefore);
console.log(JSON.stringify({parentCrossSource:true,oneTimeSourceDenied:true,nativeAccessStillTrue:true,originalFollowThroughUnchanged:true,savedNativeResultUnchanged:true,partialPendingFailedDidNotBlanketRevoke:true,followThroughSyntheticDispatches:ftCalls}));
