...
mock.module('expo-audio',()=>({setAudioModeAsync:async()=>{},createAudioPlayer:()=>{
 ...
 addListener:(_:string,fn:(s:any)=>void)=>{events.add(fn);return {remove(){events.delete(fn);}};},play(){},remove(){},pause(){}};
}}));
const {speak,stopSpeech,resetSpeech,onSpeechChange}=await import('../lib/voice');
...
fail=true;assert.equal(await speak(text,'woman-hope',callbacks),'failed');await resetSpeech();
