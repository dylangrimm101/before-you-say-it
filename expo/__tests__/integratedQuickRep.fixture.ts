...
let quickVoice=false,voiceLine='',voiceStarts=0,voiceStops=0;
mock.module('@/lib/useDictation',()=>({useDictation:()=>{
 const [status,setStatus]=React.useState('idle');
 return quickVoice?{status,error:null,cancel:async()=>setStatus('idle'),reset:async()=>setStatus('idle'),requestPermission:async()=>true,start:async()=>{voiceStarts++;setStatus('recording');},stop:async()=>{voiceStops++;setStatus('idle');return voiceLine;}}:dictation;
}}));
...
const screens:Record<string,any>={'/quick-rep/[lessonId]':QuickRep,'/saved-result':Saved,'/entry':Entry,'/continue-from-web':Login,'/account-practice':AccountPractice,'/onboarding':Onboarding,'/rehearse/[id]':Rehearse,'/debrief/[id]':Debrief,'/(tabs)':Today,'/(tabs)/library':Library,'/paywall':Paywall,'/approved-lesson/[lessonId]':Lesson,'/approved-rehearsal/[lessonId]':LessonRehearsal};
...
try{
 ...
 await go({pathname:'/approved-lesson/[lessonId]',params:{lessonId:id}});await restart();assert.ok(store.convertedLessonProgress.some((p:any)=>p.lessonId===id));assert.ok(!text().includes(opener));entry.steps.push('completed lesson revisit without private transcript');entry.status='component-local';entry.liveSemanticStatus=id==='m1-l2'?'NO-GO: unresolved Ravi attribution; offline fixtures do not cure it':'not verified';
  ...
  quickVoice=true;await go({pathname:'/quick-rep/[lessonId]',params:{lessonId:'m1-l1'}});
  assert.ok(!text().includes('Quick Rep is unavailable.'));
  for(const [index,line] of ['Can you send the handoff by Thursday?', 'Please send the handoff by Thursday so I can review it.'].entries()){
    voiceLine=line;await press('Record Quick Rep');await press('Stop and review Quick Rep');
    assert.equal(root.root.findAllByType('input').find((n:any)=>n.props.accessibilityLabel==='Quick Rep transcript').props.value,line);
    await press(index===0?'Check my rep':'Complete quick rep');
    if(index===0){assert.ok(text().includes('One cue'));await press('Try the same moment again');}
  }
  assert.ok(text().includes('You gave the move another try.'));
  assert.equal(store.drillLog.filter((x:any)=>x.drillId==='quick-rep-m1-l1').length,1);
  assert.equal(voiceStarts,2);assert.equal(voiceStops,2);quickVoice=false;
  const quickLog=structuredClone(store.drillLog);await press('Back to Today');await restart();assert.deepEqual(store.drillLog,quickLog);
  console.log('PASS actual Release Quick Rep first -> feedback -> same prompt retry -> completion -> owner remount; modeled recorder/transcription via real voice-first controls; real paid route/SQL/provider codec; synthetic response');
 ...
 assert.equal(providerCalls,outputMode==='native'?7:4,'exact ordinary provider dispatch count; no restoration or missing-owner regeneration');
