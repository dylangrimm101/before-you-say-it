...
process.env.EXPO_PUBLIC_NATIVE_BILLING_ORIGIN='https://beforeyousayit.app';params={id:guestRunId};await mount(Debrief);
...
assert.equal(root.root.findAllByType('button').filter((n:any)=>n.props.label==='Practice this conversation again').length,0,'terminal free allocation cannot advertise fresh issuance');
await press('Back to today');assert.equal(route,'/(tabs)');
await act(async()=>root.unmount());client.clear();console.log('PASS normal terminal insufficient result preserves scoreless display, no impossible restart');process.exit(0);
}
