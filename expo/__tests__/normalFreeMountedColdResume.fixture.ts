...
mock.module("@/lib/purchases", () => ({trialEligibility: async () => 0,  PRO_ENTITLEMENT: "pro", identifyPurchasesUser: async () => null, clearPurchasesIdentity: async () => {}, useIsPro: () => false, useCustomerInfo: () => ({ data: null, isLoading: false }), useOfferings: () => ({ data: null, isLoading: false }), usePurchasePackage: () => ({ isPending: false, mutateAsync: async () => { throw Error("No purchases in fixture"); } }), useRestorePurchases: () => ({ isPending: false, mutateAsync: async () => false }) }));
