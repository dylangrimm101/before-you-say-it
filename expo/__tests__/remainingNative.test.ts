...
describe("remaining native acquisition and paid experience", () => {
  ...
  test("the free result follows the working web baseline, rewrite, shift, and trial sequence", async () => {
    ...
    expect(results.indexOf('label="See what changes with practice"')).toBeLessThan(results.indexOf("STARTING INDEX"));
    ...
    expect(results).toContain("<ResultCardStack");
    expect(results).toContain("YOUR STARTING INDEX");
    expect(results).toContain('label="See my practice plan"');
    expect(results.indexOf("Here’s what practice is helping you say")).toBeLessThan(results.indexOf("<ResultCardStack"));
    expect(results).not.toContain("Try my path free for 7 days");
    ...
    expect(results).toContain('source: "debrief"');
    ...
    expect(paywall).toContain('style={styles.monthHero}');
  ...
  test("Purchased carries the actual free result and adds no history", async () => {
    ...
    expect(purchased).toContain("You’re in.");
    expect(purchased).toContain("continuity.indexValue");
    expect(purchased).toContain("continuity.observedCount");
    expect(purchased).toContain("continuity.firstFocusLabel");
    expect(purchased).not.toContain("saveActivePracticeSession");
