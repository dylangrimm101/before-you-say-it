...
export function guestVisitMessage(status: string): string {
  if(['visit_limit','exhausted','recording_limit'].includes(status))return 'The free-practice limit has been reached. Starting over does not reset it.';
...
export const canRetryGuestVisit = (status: string) => ['pending','unavailable','proof_expired','conflict'].includes(status);
