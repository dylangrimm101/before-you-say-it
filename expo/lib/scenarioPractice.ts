...
export function normalizeScenarioPracticeRun(value: unknown): PersistedScenarioPracticeRun | null {
  ...
  if (run.coachingObservation !== undefined) {
    ...
    if (![1, 3].includes(observation.coachedBeat as number) || !nonEmpty(observation.selectedDimension) || !["met", "not_met"].includes(String(observation.status)) || !nonEmpty(observation.evidenceQuote)) return null;
    canonicalRun.coachingObservation = {
      coachedBeat: observation.coachedBeat as 1 | 3,
  ...
  if (run.approvedRehearsal !== undefined) {
    ...
    if (pushbackOne === null || pushbackTwo === null || item.secondResponseAttempt !== undefined) return null;
    if (item.coachedBeat !== undefined && ![1, 3].includes(item.coachedBeat as number)) return null;
    if (item.selectedDimension !== undefined && !nonEmpty(item.selectedDimension)) return null;
    if (item.replayTarget !== undefined && !["top_of_scene", "pushback_one"].includes(String(item.replayTarget))) return null;
    ...
    const lesson: NonNullable<PilotDayRun["approvedRehearsal"]> = {
      ...
      ...(item.coachedBeat !== undefined ? { coachedBeat: item.coachedBeat as 1 | 3 } : {}),
    ...
    if ((pushbackOne || pushbackTwo) && !opener) return null;
...
export function approvedRehearsalReplayPressure(run: PilotDayRun): ScenarioCounterpartTurn | undefined {
  return run.approvedRehearsal?.coachedBeat === 3 ? run.approvedRehearsal.pushbackOne : undefined;
}
...
export function attachApprovedRehearsalCoaching(value: PersistedScenarioPracticeRun, note: string, retryInstruction: string, behaviorId: PilotBehaviorId, observation: PilotCoachingObservation, now: number): PersistedScenarioPracticeRun {
  const lesson = value.run.approvedRehearsal;
  if (!lesson?.pushbackTwo || !value.run.responseAttempt || !value.run.attempt) return value;
  const updatedAt = Math.max(now, value.run.updatedAt + 1);
  return { ...value, run: { ...value.run, state: "hope_coaching", coachNote: note, retryInstruction, coachedBehaviorId: behaviorId, coachedSegment: observation.coachedBeat === 1 ? "opener" : "pushback_response", coachingObservation: observation, approvedRehearsal: { ...lesson, beat: 5, coachedBeat: observation.coachedBeat, selectedDimension: observation.selectedDimension }, updatedAt } };
}
...
export function stageApprovedRehearsalReplay(value: PersistedScenarioPracticeRun, now: number): PersistedScenarioPracticeRun {
  ...
  const replayTarget = lesson.coachedBeat === 1 ? "top_of_scene" : "pushback_one";
  const pressure = replayTarget === "pushback_one" ? lesson.pushbackOne : undefined;
