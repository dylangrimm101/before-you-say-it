...
describe("the selected Hope or Adam role reaches speech", () => {
  ...
  it("resolves contextual paid-scenario voice at both playback boundaries", async () => {
    const source = await Bun.file(`${import.meta.dir}/../components/ScenarioPaidPractice.tsx`).text();
    expect(source).toContain("voiceForScenario(scenario");
    expect(source.match(/speak\(pressure(?:Text|\.text), counterpartVoice\)/g)).toHaveLength(2);
    expect(source).not.toContain('speak(pressureText, "woman-hope")');
    expect(source).not.toContain('speak(pressure.text, "woman-hope")');
  });
