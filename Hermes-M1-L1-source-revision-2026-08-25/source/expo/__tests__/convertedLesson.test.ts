...
function fixtureDeck(duplicateCompletion = false): string {
  const cardEntries = Array.from({ length: 22 }, (_, index) => `  { n:${index + 1}, type:'Card ${index + 1}' }`);
...
describe("accepted M1 L1 narrow correction", () => {
  ...
  test("hard-caps retries at two and preserves exact pressure/audio identity", () => {
    ...
    value = preserveM1L1Retry(value, "Second and final retry.", 13);
  ...
  test("approved named-move persistence unlocks Card 21 without filling custom blanks", async () => {
    const screen = await Bun.file(`${import.meta.dir}/../app/approved-lesson/[lessonId].tsx`).text();
    expect(screen).toContain("data-approved-move-satisfies-gate");
    expect(screen).toContain("Keep the approved move only");
    expect(screen).toContain("The approved move is already saved");
    expect(screen).not.toContain("setCustomDraft(M1_L1_CONVERSION.namedMove)");
  });
