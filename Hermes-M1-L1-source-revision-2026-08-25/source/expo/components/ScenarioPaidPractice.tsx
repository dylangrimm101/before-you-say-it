...
function SharedScenarioPaidPractice({ scenario, requestedRunId, convertedLesson, onReturnToDeck, onDiscard, onSafetyExit }: ScenarioPaidPracticeProps) {
  ...
  const { activeScenarioRun, profile, saveActiveScenarioRun, upsertSession } = useStore();
  const counterpartVoice = voiceForScenario(scenario, profile?.persona ?? "woman-hope");
  ...
  const confirmOpening = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      await speak(pressureText, counterpartVoice);
    } catch {
