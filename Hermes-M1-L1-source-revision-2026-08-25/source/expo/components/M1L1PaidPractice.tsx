...
export function M1L1PaidPractice({ requestedRunId, convertedLesson, onReturnToDeck, onDiscard, onSafetyExit }: M1L1PaidPracticeProps): React.JSX.Element {
  ...
  const currentPressure = lesson?.coachedBeat === 3 || lesson?.coachedBeat === 1 ? lesson.pushbackOne : lesson?.pushbackTwo;
  ...
  const replayFlaggedPressure = useCallback(async (isFinal: boolean): Promise<void> => {
  ...
  return <View style={styles.root}><Backdrop />
    ...
    <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <ScrollView contentContainerStyle={[styles.scroll, { paddingBottom: insets.bottom + 120 }]} keyboardShouldPersistTaps="handled">
        ...
        {state === "final_retry_available" && currentPressure ? <Reveal><StatusPill label="Optional final retry · 2 of 2" tone="amber" /><Text style={styles.title}>One final retry is available.</Text><Text style={styles.body}>Replay the same exact pressure before final capture, or continue to the behavior-only comparison.</Text><PrimaryButton label="Replay the exact pressure" onPress={() => void replayFlaggedPressure(true)} containerStyle={styles.action} /><Pressable onPress={() => void persist(transitionScenarioPracticeRun(value, "attempt_comparison", Date.now()))} style={styles.secondary}><Text style={styles.secondaryText}>Continue without another retry</Text></Pressable></Reveal> : null}
        {state === "ready_for_final_retry_capture" ? <Reveal><StatusPill label="Exact pressure replayed" tone="purple" /><Capture title="Final retry" kind="final-retry" value={draft} onChange={setDraft} onRecord={requestCapture} onType={typeCapture} /></Reveal> : null}
