...
export const [StoreProvider, useStore] = createContextHook(() => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const recoveredProgress = await recoverPendingConvertedCompletion(AsyncStorage, scenarioRun);
  ...
  const clearActiveScenarioRunStrict = useCallback(async (expected: ActiveRunRevision): Promise<void> => {
  ...
  const writePendingConvertedLessonCompletion = useCallback(async (record: ConvertedLessonProgress): Promise<void> => {
    await writePendingConvertedCompletion(AsyncStorage, record);
  }, []);
