...
export default function ApprovedLessonDeckScreen() {
  ...
  const handleDeckMessage = useCallback(async (raw: string): Promise<void> => {
    ...
    try {
      await finalizeConvertedLesson({
        ...
        clearActiveRunStrict: async (expectedRunId: string) => {
  ...
  }, [activeScenarioRun, clearActiveScenarioRunStrict, completionCommitted, customWording, isConverted, isReturning, params.runId, promotePendingConvertedLessonCompletion, router, writePendingConvertedLessonCompletion]);
