...
function sentenceAround(value: string, pattern?: RegExp): string {
  const transcript = cleanTranscript(value);
  const sentences = transcript.split(/(?<=[.!?])\s+/).filter(Boolean);
  const matched = pattern ? sentences.find((sentence) => pattern.test(sentence.toLowerCase())) : sentences[0];
  return (matched ?? transcript).split(/\s+/).slice(0, 10).join(" ");
}

function evidenceQuoteFor(dimension: M1L1DimensionId, transcript: string): string {
  const patterns: Partial<Record<M1L1DimensionId, RegExp>> = {
    issue_count: /\b(also|another time|everyone|other people|all the times|and)\b/,
    grounding_concreteness: /\b(last|yesterday|today|monday|tuesday|wednesday|thursday|friday|\d+|file|handoff|deadline)\b/,
    motive_character_language: /\b(always|never|lazy|selfish|don't care|doesn't care|respect me|on purpose|trying to)\b/,
    move_clarity: /\b(can you|can we|could you|could we|will you|would you|please|i need|by)\b/,
    evidence_discipline: /\b(also|another time|everyone|other people|all the times|always|never)\b/,
    park_and_return: /\b(i hear|that's fair|i understand|i get that|still|my point)\b/,
  };
  return sentenceAround(transcript, patterns[dimension]);
}

function flag(dimension: M1L1DimensionId, status: M1L1BehaviorFlag["status"], transcript: string): M1L1BehaviorFlag {
  return { dimension, status, evidenceQuote: status === "not_assessable" ? null : evidenceQuoteFor(dimension, transcript) };
}
...
export function m1L1CoachNote(confirmedTranscript: string, coachedBeat: 1 | 3 | 5 = 5): LessonCoachNote | null {
  ...
  const evidenceQuote = selectedFlag?.evidenceQuote ?? sentenceAround(transcript);
  ...
  for (const desiredStatus of ["not_met", "met"] as const) {
    for (const dimension of DIMENSION_PRIORITY) {
      for (const beat of sourceBeats[dimension]) {
        ...
        const evidenceQuote = selected.evidenceQuote ?? sentenceAround(turn.transcript);
