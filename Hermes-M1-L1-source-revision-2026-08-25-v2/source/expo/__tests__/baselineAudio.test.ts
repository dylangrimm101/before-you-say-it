...
describe("baseline audio lifecycle", () => {
  ...
  it("native stop failure still deletes created content and reports the stop failure", async () => {
    ...
    await expect(cleanupNativeRecordingStrict({
      ...
      uri: () => "file:///app/Caches/created-before-stop-failed.m4a",
  ...
  it("native delete failure remains rejected after stop and release", async () => {
    let released = false;
    await expect(cleanupNativeRecordingStrict({
      stop: async () => {}, uri: () => "file:///pending.m4a",
      discard: async () => { throw new Error("delete pending"); },
      releaseAudioMode: async () => { released = true; },
    })).rejects.toThrow("delete pending");
    expect(released).toBe(true);
  });

  it("browser stop failure retains buffered content and tracks for retry", async () => {
    let discarded = false;
    let released = false;
    await expect(cleanupWebRecordingStrict({
      stop: async () => { throw new Error("browser stop failed"); },
      discardBufferedContent: async () => { discarded = true; },
      releaseTracks: () => { released = true; },
